...
class TopicSubscriptionService:
    ...
    def subscribe(self, topic_name: str) -> TopicOperationResult:
        try:
            ...
            if principal:
                replication_op = self.replication_client.replicate_subscribe(topic_name, self.user)
            else:
                replication_op = self.replication_principal.replicate_subscribe(topic_name, self.user)
            ...
            return TopicOperationResult(False, MOMTopicStatus.TOPIC_NOT_EXIST, str(e))
    ...
    def unsubscribe(self, topic_name: str) -> TopicOperationResult:
        try:
            ...
            replication_op = False
            if principal:
                replication_op = self.replication_client.replicate_unsubscribe(topic_name, self.user)
            else:
                replication_op = self.replication_principal.replicate_unsubscribe(topic_name, self.user)
